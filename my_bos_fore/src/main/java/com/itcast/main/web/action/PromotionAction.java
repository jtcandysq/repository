...
public class PromotionAction extends BaseAction<Promotion> {
	...
	public String pageQuery() {
		...
		PageBean<Promotion> pageBean = WebClient
				.create(Constants.BOS_MANAGEMENT_HOST
						+ "/my_bos_mangement/services/promotionService/pageQuery?page="
						+ page + "&rows=" + rows)
	...
	public String showDetail() throws IOException, TemplateException {
		...
		// 如果htmlFile文件不存在，查询数据库，结合greemarker模板生成页面
		if (!htmlFile.exists()) {
			...
			Template template = configuration
					.getTemplate("promotion_datail.ftl");
			// 动态数据

			Promotion promotion = WebClient
					.create(Constants.BOS_MANAGEMENT_HOST
							+ "/my_bos_mangement/services/promotionService/promotion/"+model.getId())
